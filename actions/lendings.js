...
const lendings = [
	{
		...
		detect: [new View('FLASHLOAN_PREMIUM_TOTAL()', [], 'uint256')],
		delegate: true,
		url: 'https://github.com/aave/protocol-v2',
		fetchs: {
			deposittoken: {
				get(maps) {
					return maps.itoken ?? maps.tokens?.[0] ?? maps.token;
				},
			},
			outputtoken: (temp = new View(
				'getReserveData(address)',
				['__token__'],
				'(uint256),uint128,uint128,uint128,uint128,uint128 stable,uint40,address,address stable,address,address,uint8 id',
				7
			)),
		...
		deposit: new Call(
			...
			['__token__', '__amount__', '__user__', '0'],
			'0',
			{
				title: 'Deposit tokens to pool',
				params: [
					'Asset address',
					'Asset amount',
					'On behalf of',
			...
			new Check(getBalanceView('__account__', '__token__'), View.PASS)
		),
		approve: approve(
			'__debttoken__',
			'__aggregator__',
		...
		borrow: new Call(
			...
			['__token__', '__amount__', '1', '0', '__user__'],
			'__eth__',
			{
				title: 'Borrow from reserves',
				params: [
					'Asset address',
					'Amount',
					'Interest rate mode',
		...
		redeem: new Call(
			...
			['__token__', '__amount', '__account__'],
			'0',
			{
				title: 'Withdraw deposited tokens',
				params: ['Asset address', 'Amount', 'Receiver'],
				editable: 1,
				gas: '100000',
			}
		),
		repay: new Call(
			...
			['__token__', '__amount', '1', '__account__'],
			'__eth__',
			{
				title: 'Repay borrow',
				params: [
		...
		oracle: [new View('getPriceOracle()', [], 'address')],
		get redeemable() {
			return new View();
		},
		get available() {
				...
				get: async function (maps = {}, abi = 'lendings.a') {
					...
					try {
						...
						const oracle = con.attach(
							(maps.oracle = await Promise.any(
								this.oracles.map((view) =>
									view.get(false, addressprovider)
								)
							))
						);
						...
						maps.maxborrowable = res[2] = res[0]
							.mul(toPow(odecimals))
							.div(maps.oprice);
						maps.borrowable = res[1] = subSlippage(
							res[2],
							maps.action ?? 'borrows',
							maps.auto
						);
						debug(
							'available',
							str(maps.amount),
							str([maps.itoken, maps.otoken]),
							str([maps.iprice, maps.oprice]),
							str(res)
						);
					} catch (err) {
	...
	},
	{
		...
		detect: [new View('LENDINGPOOL_REVISION()', [], 'uint256')],
		delegate: true,
		...
		fetchs: {
			...
			outputtoken: (temp = new View(
				'getReserveData(address)',
				['__token__'],
				'(uint256),uint128,uint128,uint128,uint128,uint40,address,address,address,uint8',
				6
			)),
		...
		borrow: new Call(
			...
			['__token__', '__amount__', '0', '__user__'],
			'0',
			{
				title: 'Borrow from reserves',
				params: [
		...
		repay: new Call(
			...
			{
				title: 'Repay borrow',
				params: ['Asset address', 'Amount', 'On behalf of'],
				editable: 1,
				gas: '100000',
			},
			new Check(getBalanceView('__account__', '__token__'), View.PASS)
	...
	},
	{
		...
		ref: async function (maps = {}, index = -1) {
			...
			const isEth = (address) =>
				invalidAddresses
					.concat([getAddress('token.eth')])
					.includes(address.toLowerCase());
			const apiGetPools = async (tokens = [], res = null) =>
				((res = (
					await axios({
						url: this.poolsApi + maps.target,
						responseType: 'json',
					})
				).data) &&
			...
			try {
				if (index > 1) {
					...
					const tokens = [
						maps.tokens?.[0] ?? maps.token,
						maps.tokens?.[1] ?? maps.token,
					];
					...
					Object.entries({
						...
						([name, id]) =>
							(def[name] = def[name].update({
								target: def[id + 'target'],
							}))
					);
					// no delegate for borrowss
					(maps.action === 'borrows' ||
						maps.targets[0] != maps.targets[1]) &&
						(def.delegate = false);
					debug(
						'ref',
						'found',
						str([def.target].concat(maps.targets)),
						[index, def.delegate]
					);
				} else if (index == 1) {
					// target is cether
		...
		borrow: new Call(
			...
			new Check(temp, View.PASS, '__borrowable__')
		),
		redeem: new Call(null, 'redeem(uint256)', ['__amount__'], '0', {
			title: 'Redeem underlying tokens',
			params: ['Amount'],
			editable: 0,
			gas: '279000',
		}),
		repay: new Call(
			...
			'0',
			{
				title: 'Repay borrow on',
				params: ['For', 'Amount'],
				editable: 1,
				gas: '100000',
			},
			new Check(new View(), View.PASS)
		),
		get available() {
			return {
				get: async function (maps = {}, abi = 'lendings.c') {
					...
					// deprecated
					!state.deprecate &&
						([maps.itarget, maps.otarget] = [
							maps.itarget ?? maps.targets?.[0] ?? maps.target,
							maps.otarget ?? maps.targets?.[1] ?? maps.target,
						]);
					...
					try {
						...
						const controller = con.attach(
							(maps.controller = await con.comptroller())
						);
						//const bbr = await con.borrowRatePerBlock();
						const oracle = con.attach(
							(maps.oracle = await controller.oracle())
						);
						const [
							{ 1: icollateralfactor },
							{ 1: totalliquidity },
							...
							controller.markets(maps.itarget),
							controller.getAccountLiquidity(maps.account),
							oracle.getUnderlyingPrice(maps.itarget),
							oracle.getUnderlyingPrice(maps.otarget),
							getDecimals(maps.itoken),
							getDecimals(maps.otoken),
						]);
