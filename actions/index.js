...
const setAmount = (maps, val = maps.oamount) =>
	([maps._amount, maps.amount] = [maps.amount, val]);
...
const swaps = {
	...
	calls: async function (id, maps = {}, parent = {}, auto = false) {
		...
		//
		[maps.path, maps.oamount] = await findSwapPath(
			maps.target,
			maps.tokens,
		...
		//
		!isEth(maps.token) && calls.push(approve().update(maps));
		maps.type = callTypes[maps.tokens.indexOf(invalidAddresses[0]) + 1];
		calls.push(def[maps.type].update(maps));
		//
		setAmount(maps);
		//
		if (def.delegate) {
	...
	auto: function (id, maps = {}, parent = {}) {
		return state.config.optimizeSwaps
			? (async () => {
					debug('swaps', 'auto', id);
					//
					const def = await functions('swaps');
					...
					[maps.token, maps.otoken] = maps.tokens;
					//
					const [path, aouts] = await findSwapPath(
						maps.target,
						[maps.token, maps.otoken],
						(maps.amount = await parseAmount(
							maps.amount,
							maps.token
						)),
						false
					...
					const pairs = await Promise.all(
						path.map(
							(token, i) =>
								i != 0 &&
								findSwapPair(maps.target, [path[i - 1], token])
						)
					);
					const token0s = await Promise.all(
						pairs.map(
							(pair, i) =>
								i != 0 && contract(pair, 'swaps').token0()
						)
					);
					//
					for (let i = 1; i != path.length; i++) {
						...
						[maps.iamount, maps.oamount] = [
							maps.amounts[0],
							maps.amounts[1],
						];
						sameToken(token0s[i], path[i]) &&
							maps.amounts.reverse();
						[maps.target, maps.itoken, maps.otoken] = [
							pairs[i],
							path[i_],
							path[i],
						];
						...
						[].push.apply(
							calls,
							def.auto.map((call) => call.update(maps))
						);
					}
					//
					setAmount(maps);
					...
					if (def.delegate) {
						maps.outs.push(maps.token);
						def.delegate == 'transfer' &&
							maps.outs.push(maps.otoken);
					} else {
						throw new Error('notimpl: ' + 'auto');
					}
					return calls;
...
const providinglps = {
	...
	calls: async function (id, maps = {}, parent = {}, auto = false) {
		...
		[maps.amount, maps.amounts, maps.split] = [
			await parseAmount(maps.amount, maps.token),
			['0', '0'],
			false,
		];
		//
		...
		//
		maps.minamounts = maps.amounts.map((amount) =>
			subSlippage(amount, 'providinglps', auto)
		);
		...
		[maps.token, maps.otoken].forEach((token, i) => {
			isEth(token)
				...
				: calls.push(
						approve(token).update(
							OA(maps, { amount: maps.amounts[i] })
						)
				  ) && (maps.token = token);
		});
	...
	auto: function (id, maps = {}, parent = {}) {
		return state.config.optimizeLPs
			? (async () => {
					...
					try {
						// one input token, need to, swap half
						[maps.tokens, maps.reserves] = await findPairInfo(
							maps.otoken
						);
						sameToken(maps.token, maps.tokens[1]) &&
							maps.tokens.reverse();
						[maps.pair, maps.otoken] = [
							maps.otoken,
							maps.tokens[1],
						...
						if (false) {
							...
							maps.pair = await findPairAddress(
								maps.target,
								maps.tokens[0],
								maps.tokens[1]
							);
					...
					if (maps.split) {
						...
						debug(
							'swap.back',
							str(maps.tokens.concat(maps.amounts))
						);
					}
					//
					maps.lpamount = await lpAmount(maps.pair, maps.amounts);
					maps.oamount = subSlippage(
						maps.lpamount,
						'providinglps',
						true
					);
					...
					//
					if (isEth(maps.token)) {
						const m = { ...maps, amount: maps.amounts[0] };
						[].push.apply(
							calls,
							await wraps.auto(
								`_${getAddress('token.eth')}_${maps.token}`,
								m,
								null
							)
						);
...
const wraps = {
	...
	calls: async function (id, maps = {}, parent = {}) {
		...
		if (def.delegate)
			def.delegate == 'transfer' && maps.outs.push(maps.target);
...
const borrows = {
	...
	calls: async function (id, maps = {}, parent = {}, auto = false) {
		...
		const def = await findContract(
			maps.target,
			'lendings',
			OA(maps, { token: maps.tokens[1] }),
			lendings.mapNames
		);
		// need a reset from previous lendings calls
		([maps.itoken, maps.otoken] = maps.tokens),
			(maps.token = maps.otoken),
			(maps.eth = '0');
		if (calls.length && def) {
			const { borrow: call } = def;
			//
			[maps.iamount, maps.itarget, maps.otarget] = [
				maps.amount,
				calls.slice(-1)[0].target,
				def.target ?? maps.target,
			];
			//
