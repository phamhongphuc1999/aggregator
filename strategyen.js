...
const { axios, debug, findContract, functions, getAddress, getPrice, getProvider, getToken, IA, invalidAddresses, parseAmount, sameToken, serialize, str, toBN, toPow, ts } = helpers;
...
export async function getStrategy(id, amount = state.maps.amount) {
  ...
  return (
    await axios({
      url: `${state.config.baseAPI}/strategies/${id}?amount=${amount}&${state.config[ib] ? ib + "=1" : ""}`,
      responseType: "json",
    })
  ).data;
}
...
async function allCalls(steps, funcname, maps, mutate = (o) => o) {
  ...
  const get = async (step, i, get) => {
    ...
    //
    return id && action && (get = actions[action][funcname]) && (get = await get(id, OA(maps, props, parent))) && debug(funcname, action, time() - ms + "ms") && get.map((call) => OA(call, callprops));
  };
          ...
          await Promise.all(
            calls
              ...
              .filter((call, i, approves) => {
                ...
                return found !== -1 && found != i && (found = approves[found].params) && (index = 1) ? (found[index] = found[index].add(call.params[1])) && false : true;
              })
              .map(async (call) =>
                OA(call, {
                  _allowance: await (call.check?.view ?? allowance(call.target, maps.account, call.params[0])).get(),
                })
              )
          )
        )
...
async function sumGas(calls = []) {
  ...
  if (state.config.gasEstimate && calls?.length) {
    ...
    state.config.gasEstimateUSD && (gas.usd = gas.cost.mul((sumGas.p = sumGas.p ?? (await getPrice(invalidAddresses[0], true)))).div(toPow(18)) / 1e8);
  }
  //
  return gas;
}
...
export async function process(strategy, maps = {}, noauto = null, merge = true, logs = false) {
  ...
  [res.calls, auto.calls] = await Promise.all([
    noauto === false && state.config.noautoSkipCalls ? [] : allCalls(steps, "calls", res.maps, optimizeApproves),
  ...
  if (auto.calls?.length) {
    ...
    try {
      ...
      if (capitals) {
        transfers.ins = await Promise.all((auto.maps.approves?.length ? auto.maps.approves : []).concat(Object.keys(capitals)).map(processTransfer.bind(this, auto.maps)));
        state.config.autoApproveOrder && transfers.ins.sort((a) => (a.tx === null ? -1 : 0));
      ...
      auto.calls = auto.calls.map((call, i, arr) => OA(call, { ...(arr.length - 1 != i && { check: null }) })).map((call) => formatCall(call, auto.maps));
  ...
  if (res.calls?.length) {
    ...
    try {
      ...
      res.calls = (await Promise.all(res.calls.map((call) => call.meta()))).map((call) => formatCall(call, _maps));
    } catch (err) {
...
export async function autoAvailability(strategy) {
  ...
  const defs = await Promise.all(
    steps.map((step) => {
      ...
      return actions[action]?.find ? findContract(maps.target, action, maps) : { title: action, delegate: true };
    })
  );
  // count eligiblity
